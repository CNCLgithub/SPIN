...
# go over each user
def process_user(args, extract_img=True, protocol=1):
    user_i, dataset_path = args
...
def h36m_extract(dataset_path, out_path, extract_img=True):
    ...
    user_list = [1, 5, 6, 7, 8, 9, 11]

    task_pool = multiprocessing.Pool()
    args = [(i, dataset_path) for i in user_list]
    task_pool.map(process_user, args)
