...
class BaseDataset(Dataset):
    ...
    def __init__(self, options, dataset, dataset_id, ignore_3d=False,
                 use_augmentation=True, is_train=True, is_spin=True):
        ...
        if self.is_spin:
            ...
            # load labels
            self.labels = np.load("data/labels.npz")[dataset]
            if 'has_smpl' in self.data:
                self.has_smpl = self.data['has_smpl']
            else:
                self.has_smpl = np.ones(len(self.imgname))
            has_smpl_idcs = np.where(self.has_smpl)
            self.imgname = self.data['imgname'][has_smpl_idcs]
            self.scale = self.data['scale'][has_smpl_idcs]
            self.center = self.data['center'][has_smpl_idcs]
